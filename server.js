...
const port = 7777;
const db = 'mongodb://127.0.0.1:27017/biocad';
const Calibrations = require('./models/Calibrations');
...
app.get('/api/scales/:id', (req, res) => {
  Scales.find()
    .or([{ tam: req.params.id }, { guid: req.params.id }, { bims: req.params.id }])
    .then((data) => res.json(data))
    .catch((err) => console.log(err));
});

app.get('/api/calibrations/:scale_id', (req, res) => {
  Calibrations.find({ scale_id: req.params.scale_id })
    .then((data) => res.json(data))
    .catch((err) => console.log(err));
});

app.get('/api/hidden/fill_db_with_test_data', (req, res) => {
  Scales.remove({})
  ...
  Calibrations.insertMany([
    {
      ...
      data: '04.10.19 10:23',
      solution: [
        { number: 2000460789536, ph_value: '1.09' },
        { number: 2000460789536, ph_value: '2.00' },
        { number: 2000460789536, ph_value: '4.01' },
        { number: 2000460789536, ph_value: '7.00' },
        { number: 2000460789536, ph_value: '9.21' },
      ],
      slope: '98.7',
      offset: '-0.3',
